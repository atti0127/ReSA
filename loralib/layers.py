...
class LoRALayer():
    ...
    def lora_train(self, mode: bool = True):
        if not getattr(self, 'merge_lora_weights', True):
        ...
        if mode:
            ...
            # A disabled adapter must remain removed even when an enclosing
            # evaluation helper calls model.eval() again.
            if getattr(self, 'adapters_disabled', False):
                if self.merged and self.r > 0:
                    self.sub_lora_data()
                self.merged = False
                return
...
class LinearLoRA(nn.Linear, LoRALayer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.readout_partition = readout_partition
        self.global_rank = (r + 1) // 2 if readout_partition else r
        self.merge_lora_weights = not readout_partition
        if readout_partition and r < 2:
            raise ValueError('readout-partitioned LoRA requires rank r >= 2')
    ...
    def forward(
            self, x: torch.Tensor, readout_mask: Optional[torch.Tensor] = None,
            **kwargs):
        if self.adapters_disabled:
            return nn.Linear.forward(self, x, **kwargs)

        if self.readout_partition:
            original_output = nn.Linear.forward(self, x, **kwargs)
            if self.r <= 0:
                return original_output
            if readout_mask is None:
                raise ValueError(
                    'readout-partitioned projection requires a token mask')
            lora_input = self.dropout(x) if self.dropout is not None else x
            rank_features = F.linear(lora_input, self.w_lora_A)
            global_adjustment = F.linear(
                rank_features[..., :self.global_rank],
                self.w_lora_B[:, :self.global_rank])
            readout_adjustment = F.linear(
                rank_features[..., self.global_rank:],
                self.w_lora_B[:, self.global_rank:])
            readout_mask = readout_mask.to(
                device=x.device, dtype=readout_adjustment.dtype)
            return original_output + self.scaling * (
                global_adjustment + readout_mask * readout_adjustment)
...
class PlainMultiheadAttentionLoRA(nn.Module):
    def __init__(
            ...
            lora_alpha: int = 1, 
            dropout_rate:float = 0.,
            readout_kind: Optional[str] = None,
            readout_depth: Optional[float] = None,
            **kwargs
        ):
        super().__init__()
        ...
        self.adapters_disabled = False
        if readout_kind not in {
                None, 'visual_cls', 'visual_cls_progressive'}:
            raise ValueError(f'Unsupported readout kind: {readout_kind}')
        self.readout_kind = readout_kind
        if readout_kind == 'visual_cls_progressive':
            if readout_depth is None or not 0. <= readout_depth <= 1.:
                raise ValueError(
                    'progressive visual partition requires depth in [0, 1]')
            self.readout_depth = float(readout_depth)
        else:
            if readout_depth is not None:
                raise ValueError(
                    'readout_depth is only valid for a progressive visual partition')
            self.readout_depth = None
        ...
        def make_adapted_linear(linear):
            return LinearLoRA(
                linear,
                r=r,
                ...
                readout_partition=readout_kind is not None)

        # Init qkv as new adapted linear layers.
        for item in enable_lora:
            if item == 'q':
                self.q_proj = make_adapted_linear(self.q_proj)
            elif item == 'k':
                self.k_proj = make_adapted_linear(self.k_proj)
            elif item == 'v':
                self.v_proj = make_adapted_linear(self.v_proj)
            elif item == 'o':
                self.proj = make_adapted_linear(self.proj)
    ...
    def forward_module(
        ...
        query_readout_mask = self._build_readout_mask(
            query, tgt_len, bsz)
        key_readout_mask = self._build_readout_mask(
            key, src_len, bsz)

        def project(module, inputs, readout_mask):
            if (
                    isinstance(module, LinearLoRA)
                    and module.readout_partition):
                return module(inputs, readout_mask=readout_mask)
            return module(inputs)

        q = project(self.q_proj, query, query_readout_mask)
        k = project(self.k_proj, key, key_readout_mask)
        v = project(self.v_proj, value, key_readout_mask)
        ...
        if (
                isinstance(self.proj, LinearLoRA)
                and self.proj.readout_partition):
            output_readout_mask = query_readout_mask.reshape(
                tgt_len * bsz, 1)
            attn_output = self.proj(
                attn_output, readout_mask=output_readout_mask)
        else:
    ...
    def _build_readout_mask(self, tensor, sequence_length, batch_size):
        if self.readout_kind is None:
            return tensor.new_zeros((sequence_length, batch_size, 1))
        if self.readout_kind == 'visual_cls_progressive':
            patch_scale = 1. - self.readout_depth
            mask = tensor.new_full(
                (sequence_length, batch_size, 1), patch_scale)
        else:
            mask = tensor.new_zeros((sequence_length, batch_size, 1))
        mask[0] = 1
        return mask
